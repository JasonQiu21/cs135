...
(define (p-divides? a b)
  (cond
    [(pz? a) (cond [(pz? b) #t] [else #f])]
    [else (cond [(pz? (p% b a)) #t][else #f])]
  )
)
...
; Integer sqrt of p -- not strictly necessary, but saves compute time

; (define (psqrt p) (n->p (inexact->exact (floor (sqrt (p->n p))))))
