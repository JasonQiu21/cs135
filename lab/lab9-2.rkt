...
(define (get-vertices G)
  (cond
    [(null? G) G]
    [(null? (cdr G)) (usort (car G))]
    [else (get-vertices (cons (append (car G) (car (cdr G))) (cdr (cdr G))))]
  )
...
(define (degree G v)
  (define (helper G v c)
    (cond
      [(null? G) c]
      [(and (eq? (car (car G)) v) (eq? (car (cdr (car G))) v)) (helper (cdr G) v (+ c 2))]
      [(or (eq? (car (car G)) v) (eq? (car (cdr (car G))) v)) (helper (cdr G) v (+ c 1))]
      [else (helper (cdr G) v c)]
    )
  )
  (helper G v 0)
)
...
(define (all-even? G)
  (define (helper G vl)
    (cond
      [(null? vl) #t]
      [(odd? (degree G (car vl))) #f]
      [else (helper G (cdr vl))]
    )
  )
  (helper G (get-vertices G))
)
...
(define (adjacent G v)
  (define (helper G v c)
    (cond
      [(null? G) (usort c)]
      [(eq? (car (car G)) v) (helper (cdr G) v (append c (list (car (cdr (car G))))))]
      [(eq? (car (cdr (car G))) v) (helper (cdr G) v (append c (list (car (car G)))))]
      [else (helper (cdr G) v c)]
    )
  )
  (helper G v '())
...
;; 4 PTS
(define (connected? G)
  (or (null? G) (equal? (usort (dfs G (car (car G)))) (get-vertices G)))
)
