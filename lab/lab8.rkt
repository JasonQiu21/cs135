...
(define (wj-solvable? cA cB g)
  (cond
    [(and (> g cA) (> g cB)) #f]
    [(or (<= cA 0) (<= cB 0)) #f] ; A jug cannot have zero or negaive volume; if so, return false
    [else (divides? (gcd cA cB) g)]
  )
)
...
;; 6 PTS
(define (wj-step cA cB g fA fB)
  ; Generates the pair of numbers to be added to the list
  (cond
    [(eq? fB g) (list fA fB)]
    [(eq? fB cB) (list fA 0)]
    [(eq? fA 0) (list cA fB)]
    [(> (+ fA fB) cB) (list (- fA (- cB fB)) cB)]
    [else (list 0 (+ fA fB))]
  )
)
(define (wj-helper cA cB g fA fB)
  (define fa (car (wj-step cA cB g fA fB)))
  (define fb (car (cdr (wj-step cA cB g fA fB))))
  (cond 
    [(eq? fB g) '()]
    [else (cons (list fa fb) (wj-helper cA cB g fa fb))]
  )
)

(define (waterjug cA cB g)
  (cond
    [(wj-solvable? cA cB g) (cons '(0 0) (wj-helper cA cB g 0 0))]
    [(> cA cB) '()] ; cA is assumed to be the smaller of the two jugs, return empty list otherwise
    [else '()] ; If not solvable, return empty list
  )
)
