...
#|-------------------------------------------------------------------------------
 | Name: Jason Qiu
 | Pledge: I pledge my honor that I have abiced by the Stevens Honor System.
 |-------------------------------------------------------------------------------|#
...
(define (indegree E v)
  (cond
    [(null? E) 0]
    [(equal? v (cadr (car E))) (+ 1 (indegree (cdr E) v))]
    [else (indegree (cdr E) v)]
  )
)
...
(define (find-top G)
  (define (helper L E)
    (cond
      [(null? L) (topsort-error)]
      [(zero? (indegree E (car L))) (car L)]
      [else (helper (cdr L) E)]
    )
  )
  (helper (get-vertices G) (get-edges G))
)
...
(define (topsort G)
  (cond
    [(null? (get-vertices G)) '()]
    [else (append (list (find-top G)) (topsort (pop-vertex G (find-top G))))]
  )
)