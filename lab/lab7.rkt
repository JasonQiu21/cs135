...
(define (R^n R n)
  (cond 
    [(zero? n) R]
    [else (compose R (R^n R (- n 1)))]
  )
)
(define (helper R n)
  (cond
  [(zero? n) (R^n R n)]
  [else (union (R^n R n) (helper R (- n 1)))]
  )
)
...
(define (transitive? R)
  (cond
    [(null? (difference (transitive-closure R) R)) #t]
    [else #f]
  )
)
...
(define (equivalence? n R)
  (cond [(and (transitive? R) (symmetric? R) (reflexive? n R)) #t] [else #f])
)
...
(define (surjective? n R)
  (cond 
    [(zero? n) #t]
    [(null? (relate n (converse R))) #f]
    [else (surjective? (- n 1) R)]
  )
)
(define (involutive? n R)
  (cond [(and (injective? R) (surjective? n R) (symmetric? R)) #t][else #f])
)
