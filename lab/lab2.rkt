...
;; 2 PTS
(define (yoda words) (list (car (cdr (cdr words))) (car words) (car (cdr words))) )
...
;; 3 PTS
(define (pig-latin word) (append (cdr word) (cons (car word) '(a y))))
...
;; 1 PTS
(define (get-gpa s)
  (car (reverse (car (cdr (cdr(reverse s))))))
)
...
;; 2 PTS
(define (get-state s)
  (car (cdr (reverse (car (cdr (get-address s)))))))
...
(define (combine-courses s1 s2)
  (define c1 (car (reverse s1)))
  (define c2 (car (reverse s2)))
  (append c1 c2))
